...
def get_cat_chart(entered_site,entered_payload):
    min_payload=min(entered_payload)
    max_payload=max(entered_payload)
    filtered_df = spacex_df[spacex_df['Payload Mass (kg)'].between(int(min_payload),int(max_payload))]
    if entered_site == 'ALL':
        fig = px.scatter(x=filtered_df['Payload Mass (kg)'],
        y = filtered_df['class'], 
        color=filtered_df['Booster Version Category'],
        labels=dict(x='Payload Mass (kg)', y='class', color='Booster Version Category'),
